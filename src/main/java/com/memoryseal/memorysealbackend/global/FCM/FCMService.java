...
public class FCMService {
    ...
    private void sendNotification(String fcmToken, String title, String body, Long capsuleId) {
        if(fcmToken == null) {
            return;
        }
        try {
    ...
    public void sendBuriedNotification(String fcmToken, String title, Long capsuleId) {
        sendNotification(fcmToken, "타임 티켓 묻기", title + "티켓이 봉인됐어요!", capsuleId);
    }

    public void sendOpenedNotification(String fcmToken, String title, Long capsuleId) {
        sendNotification(fcmToken, "타임 티켓 개봉", title + "티켓을 확인해보세요!", capsuleId);
    }

    public void sendJoinRequestNotification(String fcmToken, String title, String nickname, Long capsuleId) {
        sendNotification(fcmToken, "타임 티켓 참가", title + "티켓에 " + nickname + "님이 참가했어요!", capsuleId);
    }
}
